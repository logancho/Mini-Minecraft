...
#include "scene/inventorybar.h"
#include "scene/blockselection.h"
#include "scene/bear.h"
...
private:
    ...
    ShaderProgram m_progLava; // shader program for when in Lava
    ShaderProgram m_progWater; // shader program for when in Water
    ShaderProgram m_progNothing;

    std::shared_ptr<Texture> terrainTexture;
    std::shared_ptr<Texture> playerTexture;
    std::shared_ptr<Texture> bearTexture;
    std::shared_ptr<Texture> birdTexture;
    ...
    InputBundle inputB; // Input Bundle at any given moment
    ...
    Bear m_bear;
    Bird m_bird;
    bool terrainReadyForNPCs = false;
...
public:
    ...
    void traverse(const uPtr<Node>& cur, glm::mat4 t, int textureSlot);
...
protected:
    ...
    void addAdjacentBlock(glm::vec3 offset);
