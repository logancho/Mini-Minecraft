...
private:
    ...
    std::unordered_set<Chunk*> need_VBO; // SHARED
    std::unordered_set<VBOData*> vbo_data; // SHARED
    std::unordered_set<VBOData*> vbo_dataT; // SHARED
    std::unordered_set<glm::ivec2*> tree; // SHARED
    std::queue<glm::ivec2*> flat_data; // SHARED

    std::priority_queue<std::pair<int, glm::ivec2*>> cactusHeap;

    QMutex lock_BlockType;
    QMutex lock_VBO;
    QMutex lock_Tree;
    QMutex lock_Plateau;
    ...
    std::unordered_set<int64_t> border(int64_t center);
    std::unordered_set<Chunk*> borderingChunks(int64_t center);
...
public:
    ...
    // ShaderProgram
    void expand(glm::vec2 curr_pos, glm::vec2 &prev_pos);
    void draw(glm::vec2 curr_pos, ShaderProgram *shaderProgram);
    void create(glm::ivec2 start);

    void spawnBlockTypeWorker(int64_t key);
    void spawnVBOWorker(Chunk* c);
    // Initializes the Chunks that store the 64 x 256 x 64 block scene you
    // see when the base code is run.

    void appendEditedChunk(Chunk* c);
    void spreadFlat();
    void buildcactus();
    void placeCactus(int x, int y, int z, LNode* system);
    void placeBlock(glm::ivec3 start, int x, int y, int z, BlockType t);
    void drawFlatCactus(glm::ivec3 curr);

    int toLocal(int i);
    bool hasNoCactus(glm::ivec2 coord);

    std::queue<Chunk*> toCreate;
};
