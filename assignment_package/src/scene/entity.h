...
struct InputBundle {
    ...
    InputBundle()
        : wPressed(false), aPressed(false), sPressed(false),
          dPressed(false), ePressed(false), qPressed(false), spacePressed(false), spaceHeld(false), flightMode(true), thirdPerson(false),
...
struct Coordinate {
    ...
    Coordinate(float xI, float yI)
        : x(xI), y(yI)
    {}
};

enum MobDirection : unsigned char
{
